...
std::shared_ptr<Curve1d> PiecewiseBrakingTrajectoryGenerator::Generate(
  ...
  /**
   * d_comfort是一个比较舒适的加速度
   * 按照d_comfort做匀减速得到comfort_stop_dist
   * **/
  ...
  /**
   * comfort_stop_dist > s_dist说明安照d_comfort作匀减速不能在停止点前停下
   * 则不能以舒适的加速度进行停车，调整加速度(stop_d)，做匀减速运动
   *
   * **/
  if (comfort_stop_dist > s_dist) {
    ...
    /**
     * max_time一般为8秒
     * 如果ptr_trajectory->ParamLength() < max_time
     * 假设ptr_trajectory->ParamLength()=6秒
     * 剩下的2秒让车保持停止状态(即第6秒时的状态)
     * **/
  ...
  /**
   * comfort_stop_dist <=
   * s_dist说明安照d_comfort作匀减速可以在停止点前停下，甚至提前停车
   *
   * 如果v_curr > v_target，将减速过程分成三段：
   * 1. 匀减速 v_curr-->v_target  加速度-d_comfort，用时t_rampdown
   * 2. 匀速巡航v_target 加速度0，用时t_cruise
   * 3. 匀减速 v_target-->0 加速度-d_comfort，用时t_dec
   * **/
  if (v_curr > v_target) {
    ...
    if (s_rest > 0) {
      /**
       *
       * 如果v_curr < v_target 且 s_rest>0 ，将变速过程分成三段：
       * 1. 匀加速 v_curr-->v_target  加速度a_comfort，用时t_rampup
       * 2. 匀速巡航v_target 加速度0，用时t_cruise
       * 3. 匀减速 v_target-->0 加速度-d_comfort，用时t_dec
       * **/
      ...
      /**
       *
       * 如果v_curr < v_target 且 s_rest<=0 ，将变速过程分成两段：
       * 1. 匀加速 v_curr-->v_max  加速度a_comfort，用时 t_acc
       * 2. 匀减速 v_target-->0 加速度-d_comfort，用时t_dec
       * **/
