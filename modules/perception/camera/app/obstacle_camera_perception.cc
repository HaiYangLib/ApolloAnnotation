...
/**
 * 参数options由FusionCameraDetectionComponent模块提供
 * FusionCameraDetectionComponent模块根据其配置文件填充
...
bool ObstacleCameraPerception::Init(
   ...
   *            camera/models/yolo_obstacle_detector"
   ...
   *     name : "YoloObstacleDetector"
   *     root_dir :
   *     "/apollo/modules/perception/production/data/
   *            perception/camera/models/yolo_obstacle_detector"
   *     config_file : "config.pt"
   *   }
   *   camera_name : "front_12mm"
   * }
   ***/
  // 步骤2：遍历detector_param,
  // 步骤2.1：创将畸变模型，
  // 步骤2.2：创建YoloObstacleDetector
  for (int i = 0; i < perception_param_.detector_param_size(); ++i) {
    ...
    /**
     * name : "YoloObstacleDetector"
     * 创建一个实例YoloObstacleDetector
     * 在下面文件中定义：
     * modules/perception/camera/lib/obstacle/detector/yolo/
   ...
   *     root_dir :
   *        "/apollo/modules/perception/production/data/
   *              perception/camera/models/omt_obstacle_tracker"
   *     config_file : "config.pt"
   *   }
   * }
   * ...........................
  ...
  ACHECK(perception_param_.has_tracker_param()) << "Failed to init tracker.";
  {
    ...
    /**
     * 
     * plugin_param.name() : "OMTObstacleTracker"
     * 创建OMTObstacleTracker
     * 在下面文件中定义：
     * modules/perception/camera/app/obstacle_camera_perception.cc
     * 
     * **/
    // 步骤3： 创建OMTObstacleTracker并初始化
    tracker_.reset(
   ...
   *     root_dir :
   *        "/apollo/modules/perception/production/data/
   *              perception/camera/models/multicue_obstacle_transformer"
   *     config_file : "config.pt"
   *   }
   * }
   * **/
      ...
      << "Failed to init transformer.";
  {
    ...
    /**
     * plugin_param.name()："MultiCueObstacleTransformer"
     * 创建MultiCueObstacleTransformer实例
     * 在下面文件中定义
     * modules/perception/camera/lib/obstacle/transformer/multicue/
   ...
   *     root_dir :
   *        "/apollo/modules/perception/production/data/
   *           erception/camera/models/location_refiner_obstacle_postprocessor"
   *     config_file : "config.pt"
   *   }
   * }
   *
      ...
      << "Failed to init obstacle postprocessor.";
  {
    ...
    /**
     * plugin_param.name()="LocationRefinerObstaclePostprocessor"
     * 创建 LocationRefinerObstaclePostprocessor实例
     * 在下面文件中定义：
     * modules/perception/camera/lib/obstacle/postprocessor/
     *        location_refiner/location_refiner_obstacle_postprocessor.cc
     * **/
    // 步骤4: 创建LocationRefinerObstaclePostprocessor实例，并初始化
  ...
  /**
   *
   * perception_param_.has_feature_param()=false
   * **/
 ...
 *       root_dir : "/apollo/modules/perception/production/data/
 *                            perception/camera/models/lane_detector/"
 ...
 *     root_dir : "/apollo/modules/perception/production/data/perception/
 *                            camera/models/lane_postprocessor/darkSCNN/"
 *     config_file : "config.pt"
 *   }
 * }
 * 
 * 另一个lane_param是对front_12mm的配置，除了camera_name其他参数都一样
 * 
 * 我推测实际上两个lane_param中只有一个会生效
 * 因为lane_detector_会在for中赋值两次，第二次会把第一次覆盖
 * **/
void ObstacleCameraPerception::InitLane(
  ...
  for (int i = 0; i < perception_param.lane_param_size(); ++i) {
    ...
    /**
     * 根据camera_name得到一个BaseCameraModelPtr
     * 实际指向PinholeCameraModel
     *  
     * **/
    base::BaseCameraModelPtr model =
    ...
    /**
     * lane_detector_plugin_param.name()="DarkSCNNLaneDetector"
     * 得到一个DarkSCNNLaneDetector实例
     * 在下面文件中定义：
     * modules/perception/camera/lib/lane/detector/darkSCNN/
    ...
    /**
     * name : "DarkSCNNLanePostprocessor"   
     * 得到一个DarkSCNNLanePostprocessor实例
     * 在下面文件中定义：
     * modules/perception/camera/lib/lane/postprocessor/
     *      darkSCNN/darkSCNN_lane_postprocessor.cc
     * **/  
